describe PortScanner::Cidr do
  ...
  context 'Work Creation' do
    it 'Enqueues a work object for each host, protocol, and port for the arguments' do
      ...
      subject.new(input_queue: input_queue, output_queue: output_queue, cidr: '127.0.0.1', ports: (20..22), protocols: ['tcp']).setup
    end
    ...
    it 'Accepts CIDR ranges and parses them accordingly' do
      ...
      subject.new(input_queue: input_queue, output_queue: output_queue, cidr: '192.168.0.20/30', ports: (22..22), protocols: ['tcp']).setup
    end
